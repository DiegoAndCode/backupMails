...
def save_folder_emails(imap_conn, folder_name, mbox):
    # Selecionar a pasta
...
def count_emails_in_folder(imap_conn, folder_name):
    # Selecionar a pasta
    status, messages = imap_conn.select(folder_name)
    if status != 'OK':
        print(f"Erro ao selecionar a pasta {folder_name}")
...
def save_all_folders_emails(imap_conn, mbox):
    # Salvar e-mails da caixa de entrada (INBOX)
    inbox_count = count_emails_in_folder(imap_conn, 'INBOX')
    if inbox_count >= 0:
        print(f"Processando pasta: Caixa de Entrada ({inbox_count} e-mails)...")
        save_folder_emails(imap_conn, 'INBOX', mbox)
    ...
    for folder in folders_sorted[1:]:
        ...
        folder_count = count_emails_in_folder(imap_conn, folder_name)
        if folder_count >= 0:
            print(f"Processando pasta: {folder_name} ({folder_count} e-mails)...")
            # Salvar e-mails da pasta
            save_folder_emails(imap_conn, folder_name, mbox)
...
def main():
    ...
    for user, password in emails.items():
        username = user.split('@')[0]

        # Criar o caminho do arquivo mbox
        mbox_path = os.path.join(current_directory, fr'BackupsEmail\bkp_{username}.mbox')
        ...
        save_all_folders_emails(mail, mbox)
